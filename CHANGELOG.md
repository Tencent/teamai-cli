...
- MCP `requires` is resolved from `PATH` (including Windows `PATHEXT`), so `teamai mcp inject` no longer skips servers such as `uvx` on Windows ([#539](https://github.com/Tencent/teamai-cli/issues/539)).
