...
- The lock behind `pull`, `push`, the reports and learnings worktrees, learnings publishing, migration, self-mode bootstrap and the update check no longer hands one lock to two live processes. Reclaiming a stale lock renamed over whatever file was there once it had judged the lock stale, and it judged two live states stale: a lock that had just been released (and could be re-created by a third process before the rename), and a lock whose owner had created it but not yet written it. Under 16 processes contending on one lock, about 1% of acquisitions overlapped another holder, enough for two pulls to report the same usage twice. Neither is renamed over any more: a lock that vanished gets one more exclusive create, and a new lock is published with its content already in place (written to a temp file, then hard-linked to the lock name), so it is never empty. On a filesystem without hard links the create falls back to the previous one: there an empty lock reads as held until it has stayed empty for 5 seconds (its owner died before writing it), and a creator keeps the lock only if it wrote it within 2.5 seconds, so none that stalled can be taken over and still report success. A lock whose owner is alive but runs as another user (for example a `sudo teamai` run), or that cannot be read at all, also reads as held instead of stale; an unreadable one is named in a warning so it can be removed by hand. Migration skips the lock's temporary files, which a contending pull creates and removes while the copy runs. With the change, the same stress run shows no overlap (for [#760](https://github.com/Tencent/teamai-cli/issues/760)).
