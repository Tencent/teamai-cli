...
export function formatIndividualComment(
  ...
  if (provider === 'github') {
    ...
    lines.push('> 💡 确认写入此条？请点击右上角"解决"按钮');
    lines.push(`> 💡 不写入？保持未解决状态即可`);
  }
  lines.push('');
  lines.push('_Auto-generated by `teamai ci extract-mr`_');
 ...
 * TGit 上作为 code review comment（关联到 MR 变更文件第 1 行），带"解决"按钮。
 */
export async function postIndividualComments(
  ...
  if (parsed.provider === 'github') {
    ...
    // 获取 MR 变更文件列表，取第一个文件作为 comment 锚点
    const changesResp = await tgitRequest(
      `/projects/${projectId}/merge_requests/${mrGlobalId}/changes`,
      'GET',
    );
    let anchorFile = 'README.md';
    if (changesResp.ok) {
      const changesData = (await changesResp.json()) as { changes?: Array<{ new_path: string }> };
      if (changesData.changes && changesData.changes.length > 0) {
        anchorFile = changesData.changes[0].new_path;
      }
    }
