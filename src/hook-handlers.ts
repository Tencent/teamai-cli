    ...
    try {
      ...
      if (usesBranchWorktree(localConfig)) {
        ...
        try {
          ...
          // Push failed — will retry next session. But it must not be silent:
          // a swallowed team-sync failure is indistinguishable from "nobody
          // upvoted", which is how #723's structurally-zero `upvoted_count`
          // stayed undiagnosable. This catch covers both the pending-delta read
          // and the push itself — either way the votes did not reach the team.
          log.debug(`votes-sync failed (worktree): ${e instanceof Error ? e.message : String(e)}`);
        }
      } else {
        await syncVotesToTeam(localConfig.repo.localPath, localConfig.username, votesDir).catch((e) => {
          // Same contract as the worktree path above: retry next session, but
          // the reason must reach debug.log.
          log.debug(`votes-sync failed: ${e instanceof Error ? e.message : String(e)}`);
        });
      }
      ...
      // Format defensively: a `throw 'string'` / `throw null` would make
      // `(e as Error).message` undefined, and `null.message` would throw inside
      // this catch and mask the original failure. Matches the convention used
      // by hook-dispatch-cli's own catch.
      log.debug(`votes-sync failed: ${e instanceof Error ? e.message : String(e)}`);
    }
    return null;
  },
