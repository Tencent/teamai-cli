...
interface HttpProviderExecutionContext {
  name: string;
  home: string;
...
/** Run the legacy ClawPro client with state isolated to one named HTTP provider. */
export async function withLocalAgentProvider<T>(
  provider: HttpProviderExecutionContext,
  operation: () => Promise<T>,
): Promise<T> {
  return httpProviderContext.run(provider, operation);
}
...
function getErrorLogPath(): string {
  const provider = httpProviderContext.getStore();
  return provider ? path.join(provider.home, REPORTER_ERROR_LOG) : path.join(getTeamaiHomePath(), REPORTER_ERROR_LOG);
}
...
function localAgentTag(context: LocalAgentContext): string {
  ...
  const provider = httpProviderContext.getStore();
  return `${provider ? `[provider:${provider.name}] ` : ''}[${resolveLocalAgentId(context).slice(-6)}] [${tool}]`;
}
...
export async function loadLocalAgentConfig(): Promise<LocalAgentConfig | null> {
  if (!httpProviderContext.getStore() && await pathExists(path.join(getLocalAgentHome(), 'migrated-to'))) {
    return null;
  }
  const fileConfig = await readJson<LocalAgentConfig>(getConfigPath());
  if (fileConfig?.endpoint) {
    const providerCredential = httpProviderContext.getStore()?.credentialPath;
    const storedToken = providerCredential ? (await readFileSafe(providerCredential))?.trim() : undefined;
    const config = {
      ...fileConfig,
      token: storedToken || fileConfig.token,
  ...
  // Named providers never borrow legacy global config or environment settings.
  if (httpProviderContext.getStore()) return null;
...
async function getResourceRepoPath(scope: LocalAgentScope, workspacePath?: string): Promise<string> {
  if (scope === 'project' && workspacePath) {
    ...
    const provider = httpProviderContext.getStore();
    const relativeHome = provider
      ? path.join('providers', 'http', provider.name)
      : LOCAL_AGENT_DIR;
    return path.join(dataHome, 'workspaces', managedMcpWorkspaceId(workspacePath), relativeHome, 'resources');
  }
...
async function maybeReconcilePlugins(context: LocalAgentContext): Promise<void> {
  try {
    ...
    const providerName = httpProviderContext.getStore()?.name;
    const args = [process.argv[1], 'source', 'reconcile-plugins'];
...
export async function initLocalAgentHttp(options: {
  ...
  const providerContext = httpProviderContext.getStore();
  await saveLocalAgentConfig(providerContext ? { ...config, token: undefined } : config);
  if (options.token) {
    await writeTokenFile(providerContext?.credentialPath ?? TEAMAI_TOKEN_PATH, options.token);
  }
  ...
  log.success(provider
    ? `HTTP provider "${provider.name}" initialized at ${getConfigPath()}`
    : `HTTP local agent initialized at ${getConfigPath()}`);
}
